...
| **BUG** | BUG-007 | Дублирование "Описание проекта" | 1h | 🔴 HIGH | ✅ DONE |
| **BUG** | BUG-008 | social_links array→object | 1h | 🔴 HIGH | ✅ DONE |
| **BUG** | BUG-009 | Улучшены ошибки валидации | 0.5h | 🟡 MEDIUM | ✅ DONE |
...
| ID | Проблема | Решение |
|----|----------|---------|
| BUG-007 | Дублирование поля "Описание проекта" в Full Analysis (спрашивается на шаге 1 и 2) | Заменено на подсказку "💡 Описание проекта будет взято из бизнес-описания (Шаг 1)" |
| BUG-008 | `social_links: Expected object, received array` при Gap Detection с конкурентами | Трансформация массива `[{url, platform}]` в объект `{twitter: '...'}` перед отправкой |
| BUG-009 | Непонятные ошибки валидации без указания поля | Добавлен путь к полю в сообщение (например `competitors.0.social_links`) |
