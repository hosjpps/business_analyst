...
test.describe('Code Analysis Flow', () => {
  ...
  test('should display code analysis mode option', async ({ page }) => {
    // Проверяем наличие режима "Проверка кода"
    const codeMode = page.locator(
      '[data-testid="mode-code"], button:has-text("Проверка кода"), button:has-text("Code"), .mode-code'
    );
  ...
  test('should show GitHub URL input when code mode selected', async ({
    ...
    const githubInput = page.locator(
      '[data-testid="github-url"], input[placeholder*="github"], input[name="github"], #github-url'
    );
  ...
  test('should show progress indicator when analysis starts', async ({
    ...
    const progressIndicator = page.locator(
      '[data-testid="progress-indicator"], .progress-indicator, .loading-indicator, .spinner'
    );
  ...
  test.skip('should complete code analysis and show results', async ({
    ...
    // Этот тест лучше запускать отдельно или в CI с увеличенным таймаутом

    const codeMode = page.locator(
      '[data-testid="mode-code"], button:has-text("Проверка кода"), .mode-code'
    );
    await codeMode.first().click();
    ...
    const results = page.locator(
      '[data-testid="code-analysis"], .code-analysis-results, .analysis-results'
    );
    ...
    const stage = page.locator(
      '[data-testid="project-stage"], .project-stage, .detected-stage'
    );
    const tasks = page.locator(
      '[data-testid="tasks-list"], .tasks-list, .task-item'
    );
  ...
  test('should show file upload option', async ({ page }) => {
    ...
    const fileUpload = page.locator(
      '[data-testid="file-upload"], input[type="file"], .file-upload, .drop-zone'
    );
  ...
  test('should show chat section after analysis', async ({ page }) => {
    ...
    const scenarios = page.locator(
      '[data-testid="demo-scenario-card"], .demo-scenario-card'
    );
    await scenarios.first().click();

    // После анализа должна появиться секция чата
    const chatSection = page.locator(
      '[data-testid="chat-section"], .chat-section, .follow-up-chat'
    );
