...
КРИТИЧЕСКИ ВАЖНО — ПОНЯТНЫЙ ЯЗЫК:
- Пиши ВСЕ тексты ПРОСТЫМ ЯЗЫКОМ, понятным человеку без технического образования
- IT термины объясняй в скобках: "добавить CI/CD (автоматическую сборку и публикацию)"
- Избегай аббревиатур без расшифровки: вместо "настроить SSR" пиши "настроить SSR (рендеринг на сервере для SEO)"
- Вместо "имплементировать" пиши "добавить/настроить"
- Вместо "интегрировать API" пиши "подключить сервис X"
- В описании задачи давай пошаговую инструкцию, которую поймёт новичок
- Примеры правильных формулировок:
  - "Настроить автоматическое тестирование (CI) — чтобы код проверялся перед публикацией"
  - "Добавить Redis (быстрая база для кэша) — ускорит загрузку страниц"
  - "Подключить Stripe (сервис оплаты) — чтобы принимать карты"
